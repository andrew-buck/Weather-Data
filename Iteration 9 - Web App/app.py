...
def data_interface():
    ...
    return flask.render_template('data-interface.html', max_id=max_id)

@app.route('/get-data')
def get_data():
    """
    API endpoint to fetch weather data based on ID range.
    """
    ...
    try:
        engine = create_engine('sqlite:///weather.db')
        with engine.connect() as conn:
            # Build query based on ID range
            query = text("SELECT * FROM weather WHERE id >= :min_id AND id <= :max_id ORDER BY id")
