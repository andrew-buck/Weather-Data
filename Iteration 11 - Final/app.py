...
# Declare a weather data variable to be initialized later
weather = None
...
def get_weather_data():
    ...
    if weather is None:
        try:
            ...
            # Update the dataframe from the database
...
def visualizer():
    ...
    """
    # Holds locations of the weather database
    locations = []
    try:
...
def generate_chart():
    ...
    try:
        ...
        if fig:
            ...
            # Return the chart to put on the web page
...
def generate_forecast():
    ...
    try:
        ...
        # Return the forecast data to put on the web page
        return flask.jsonify({ 
            'success': True,
...
def data_interface():
    ...
    """
    max_id = 0
    locations = [] # Holds locations of the weather database
    try:
        engine = create_engine('sqlite:///weather.db')
        with engine.connect() as conn:
            # Get max ID
...
def get_data():
    ...
    try:
        ...
        with engine.connect() as conn:
            ...
            # Return the requested data to put on the web page
            return flask.jsonify({ 
                'success': True,
                'columns': columns,
                'rows': rows
...
def delete_data():
    ...
    try:
        ...
        with engine.connect() as conn:
            ...
            weather.update_from_database()
            return flask.jsonify({ # Return the success message
                'success': True,
...
def summary():
    """
    Page for displaying weather data summary statistics.
    """
    # Get the weather data instance
    weather_data = get_weather_data()
    
    # Get summary statistics as a pandas DataFrame
    summary_df = weather_data._WeatherData__extracter.summary()
    
    # Convert DataFrame to HTML for display
    summary_html = summary_df.to_html(classes='data-table', border=0)
    
    return flask.render_template('summary.html', summary_table=summary_html)
